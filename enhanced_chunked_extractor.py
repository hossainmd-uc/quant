...
class EnhancedChunkedExtractor:
    """Enhanced version of the working extractor with checkpoints and all API keys"""
    
    def __init__(self, api_keys: List[str], pages_per_chunk: int = 2, checkpoint_interval: int = 20):
        ...
        print(f"🚀 Enhanced extractor initialized with {len(api_keys)} API keys, checkpoint every {checkpoint_interval} chunks")
    ...
    def _save_checkpoint(self):
        ...
        checkpoint_data = {
            ...
            "usage_trackers": [asdict(tracker) for tracker in self.usage_trackers],
    ...
    def _split_pdf(self, pdf_path: str, output_dir: str) -> List[str]:
        """Split PDF into smaller chunks using PyPDF2 (same as working original)"""
        ...
        with open(pdf_path, 'rb') as file:
            ...
            for i in range(0, total_pages, self.pages_per_chunk):
                ...
                pages_in_chunk = 0
                for page_num in range(i, min(i + self.pages_per_chunk, total_pages)):
    ...
    def _process_chunk(self, chunk_path: str, chunk_id: int) -> ChunkResult:
        """Process a single PDF chunk (same API call as working original)"""
...
def main():
    ...
    print(f"📊 Using ALL {len(api_keys)} API keys for maximum efficiency and redundancy")
    ...
    extractor = EnhancedChunkedExtractor(api_keys, checkpoint_interval=20)
